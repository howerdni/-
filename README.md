...
- **保留关键特性**：在简化电网模型的同时，保留了关键节点的动态行为和电气特性，确保仿真结果的准确性。
- **灵活性强**：支持自定义节点顺序和多端口等效计算，适应不同的仿真需求。

### **4.5 应用场景**

- **电力系统规划**：辅助进行电网扩展和优化设计，评估不同方案的可行性和经济性。
- **稳定性分析**：评估电网在不同扰动下的动态响应，确保系统的稳态和暂态稳定。
- **新能源接入评估**：分析大规模新能源（如风电、光伏）接入对电网的影响，优化新能源与电网的匹配。

通过上述等值网络算法，苏州电网 DIgSILENT 仿真平台能够高效、准确地模拟和分析复杂的电力系统，为电力规划、运行和优化提供有力支持。
...
- **BPA 的 PSS 模型 SIUI** 对应 **PSS2A（PSS/E）**
- **BPA 的原动机模型 GSTBUI 及 GS, TB 卡** 对应 **IEEEG1 模型**

在 DIgSILENT 平台上，对所有的同步机辅助控制系统均进行了模型等效替换，确保动态特性与原系统保持一致。具体对应关系如下表所示：

| BPA 模型卡 | 对应 DIgSILENT 模型 | PSS/E 模型 |
|------------|----------------------|------------|
| FV 卡 (FVUI) | ESST1A               | PSS/E      |
| FQ 卡 (FQUI) | ESAC2A               | PSS/E      |
| PSS 模型 (SIUI) | PSS2A                | PSS/E      |
| 原动机模型 (GSTBUI, GS, TB 卡) | IEEEG1               | PSS/E      |
...
- **ESAC2A（PSS/E）**：用于模拟同步机的无功调节功能，提高系统的电压稳定性。
- **PSS2A（PSS/E）**：用于模拟同步机的功率系统稳定器，增强系统阻尼，防止振荡。
...
- **常规直流输电（HVDC-LCC）**：提供详细的可控电压源模型，能够准确模拟直流电流的控制和故障响应。
- **柔性直流输电（HVDC-VSC）**：采用先进的电压源换流器模型，支持动态调节和灵活控制，适应复杂的电力传输需求。
- **风电接入模型**：
  - **双馈感应发电机（DFIG）模型**：
  - **全功率风力发电机模型**：提供全面的动态仿真能力，适用于大规模风电场的集成和调度分析。
...
- **控制模型**：
  ...
  - **涡轮模型（Turbine）**：模拟风机涡轮的动力学行为。
  - **变桨控制（Pitch Control）**：调整风机叶片角度以优化功率输出。
  - **补偿控制（Compensation）**：提供无功功率补偿，提升系统电压稳定性。
  - **保护控制中包含 Crowbar 元器件的投切逻辑**：在特定故障情况下，通过 Crowbar 电路保护发电机设备。
